...
class List(handlers.ListHandler):
    def create_base_query(self, params):
        ...
        return query
    def serialize_record(self, drug):
        return {
            'id':               drug.id,
            'code':             drug.code,
            'is_good_quality':  drug.is_good_quality
        }

class Get(handlers.GetHandler):
    def get_record(self, params):
        manager = components.DataManager()
        code = Drug.objects.get(pk=params['id'])
        drug = manager.get(code.code)
        return drug
...
class History(handlers.GetHandler):
    def get_record(self, params):
        manager = components.DataManager()
        drug_history = manager.history(params['id'], params['field'])
        return drug_history
...
class ListDetail(handlers.ListHandler):
    ...
    def serialize_record(self, drug):
        manager = components.DataManager()
        fields = manager.get(drug.code)
        data = {}
        for key, value in fields.items():
            if 'value' in value:
                data[key] = value['value']
        return {drug.code: data}
