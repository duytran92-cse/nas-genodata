...
class PageStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENODATA_API_URL + '/variation/list', GET=params)
        return data
    def get(self, id):
        return self.container.call_api(settings.GENODATA_API_URL + '/variation/get', GET={'id': id})
    def create(self, data):
        return self.container.call_api(settings.GENODATA_API_URL + '/variation/create', POST=data)
    def update(self, data, id):
        data['id'] = id
        return self.container.call_api(settings.GENODATA_API_URL + '/variation/update', POST=data)
    def history(self, id, field):
        return self.container.call_api(settings.GENODATA_API_URL + '/variation/history', GET={'id': id, 'field': field})
    def delete(self, id):
        return self.container.call_api(settings.GENODATA_API_URL + '/variation/delete', POST={'id': id})
