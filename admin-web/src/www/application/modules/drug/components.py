...
class PageStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENODATA_API_URL + '/drug/list', GET=params)
        return data
    def get(self, id):
        return self.container.call_api(settings.GENODATA_API_URL + '/drug/get', GET={'id': id})
    def create(self, data):
        return self.container.call_api(settings.GENODATA_API_URL + '/drug/create', POST=data)
    def update(self, data, id):
        data['id'] = id
        return self.container.call_api(settings.GENODATA_API_URL + '/drug/update', POST=data)
    def history(self, id, field):
        return self.container.call_api(settings.GENODATA_API_URL + '/drug/history', GET={'id': id, 'field': field})
    def delete(self, id):
        return self.container.call_api(settings.GENODATA_API_URL + '/drug/delete', POST={'id': id})
...
class FullPageContext(page_contexts.FullPageContext):
    def __init__(self, params,container):
        ...
        self.menu.set_group_selected('drug')
