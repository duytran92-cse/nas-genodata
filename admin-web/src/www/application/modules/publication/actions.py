...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/publication/update/%s">Edit</a>' % (row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/publication/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Publication')
        table.set_subtitle('List of publication')
        # table.create_button('create', '/publication/create', 'zmdi-plus')
...
class Delete(actions.crud.DeleteAction):
    def GET(self):
        ...
        return HttpResponseRedirect('/publication/list')
